...
my $mmb_dates='24-26 октября 2008';
my $reg_end_date='24:00 13 октября';
my $a_size='60 км';
my $b_size='70 км';

## файл со всеми данными.
## должен быть недоступен по http!
my $dfile = '/home/slazav/mmb_reg.dat'; 
#my $dfile = '/home/slazav/mmb_data';

## адрес скрипта
#my $script='http://localhost/mb/reg05l.pl';
my $script='http://slazav.mccme.ru/perl/reg08o.pl';
...
if ($type =~ /^dump/) {print "Content-Type: text/plain; charset=koi8-r\n\n";}
else {print "Content-Type: text/html; charset=koi8-r\n\n";}
...
if ($type eq 'dump_yaml'){
  read_data();
  for (my $i=0; $i<=$#data; $i++){
    next if !defined($data[$i]);
    next if defined(param('dist')) && param('dist')!=$data[$i]->{dist};
    print "---\n";
    print "N: $i\n";
    print "dist: $data[$i]->{dist}\n";
    print "gps: $data[$i]->{gps}\n";
    print "name: '", quote($data[$i]->{name}, "'"), "'\n";
    print "people:\n";
    foreach (1..9){
      next if $data[$i]->{"u$_"} eq '';
      print "  - '", quote($data[$i]->{"u$_"}, "'"), "'\n";
    }
  }
  exit;
}
...
if ($type eq 'dump_csv'){
  ...
  for (my $i=0; $i<=$#data; $i++){
    next if !defined($data[$i]);
    next if defined(param('dist')) && param('dist')!=$data[$i]->{dist};
    print "\"$i\", \"$data[$i]->{dist}\", \"$data[$i]->{gps}\", ";
    printf "\"%s\", ", quote($data[$i]->{name}, "\"");
    print "\"";
...
sub print_start_page{
  print qq* 
    ...
    $mmb_dates года</h3><hr>
    <h3 align=center>Регистрация команд</h3>

    <p><table border=0 cellpadding=5><tr><td valign=top>

    <p>Регистрация работает до $reg_end_date.

    <p>Прежде, чем регистрироваться для участия в марш-броске,
    прочитайте <a href="http://slazav.mccme.ru/mmb/pol08o.htm">Положение</a>.

    <p>По всем вопросам обращайтесь по адресу
    <a href="mailto:atonis</z>\@mail.ru">atonis\@mail.ru</a> (Саша Тонис).
    ...
    На один из этих адресов <a href="#pass">по специальному
    запросу</a> может быть автоматически выслан ваш пароль. Адреса не
    будут публиковаться в интернете и использоваться каким-либо другим
    образом. 
  ...
  *;
  ...
  print qq* 
    ...
    <th>Дистанция А ($a_size)</th>
    <th>Дистанция Б ($b_size)</th>
    </tr>
    <tr>
  *;
  ...
  *;
  ...
  print qq*
    ...
    <div align=right><a href="$script?type=dump_csv">[CSV]</a>
    <a href="$script?type=dump_yaml">[YAML]</a></div>
  *;
...
To: $mail
Subject: mmb password
Content-Type: text/plain; charset="koi8-r"


    Ваша команда была зарегистрирована на
    участие в Московском марш-броске
...
sub html_rr2_form{
  ...
  $out.= qq*
    ...
    Вариант дистанции:
     А ($a_size)<input name="dist" type="radio" $A value="0">&nbsp;
     Б ($b_size)<input name="dist" type="radio" $B value="1"><br><br>
    Контактный адрес:
